...
class _PatientChatRoomsScreenState extends State<PatientChatRoomsScreen> {
  ...
  Future<void> _loadRooms() async {
    ...
    try {
      final data = await _api.getChatRoomsByPatient(currentPatientId);
      if (!mounted) return;
      setState(() {
        _rooms = data;
        _isLoading = false;
      ...
      if (!mounted) return;
      setState(() {
        _error = 'Không thấy danh sách tin nhắn: $e';
        _isLoading = false;
      });
    }
  }
  ...
  //   try {
  //     await _api.createOrGetChatRoom(patientId: currentPatientId, doctorId: 1);
  //     await _loadRooms();
  //     if (!mounted) return;
  //     ScaffoldMessenger.of(context).showSnackBar(
  //       const SnackBar(content: Text('Đã tạo/lấy phòng chat với bác sĩ')),
  //     );
  //   } catch (e) {
  //     if (!mounted) return;
  //     ScaffoldMessenger.of(context).showSnackBar(
    ...
    await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => ChatDetailScreen(
          roomId: '${room['id']}',
          title:
              '${_text(room['doctorTitle'], fallback: '')} ${_text(room['doctorName'], fallback: 'Bác sĩ')}'
                  .trim(),
          senderRole: 'patient',
        ),
      ),
      ...
      return RefreshIndicator(
        ...
        child: ListView(
          children: const [
            ...
            SizedBox(height: 8),
            Center(
              child: Text('Bấm "Tạo chat test" để tạo phòng với bác sĩ 1'),
            ),
          ],
        ),
      );
