  ...
  void _handleLogin() async {
    ...
    if (!RegExp(r'^\d{6}$').hasMatch(password)) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Mật khẩu không hợp lệ')));
      return;
    }

    final user = await FirebaseAuthService.instance.loginDoctor(
      phone,
      password,
      doctorId,
    );
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: AppSpacing.xl),
              Care4uButton(text: 'Đăng nhập', onPressed: _handleLogin),
            ],
          ),
        ),
      ),
